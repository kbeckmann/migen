...
def _build_script(source, build_template, build_name, architecture,
                  basecfg, freq_constraint, idcode):
...
class LatticeTrellisToolchain:
    ...
    def __init__(self):
        ...
        self.build_template = [
            "echo 'Executing [yosys -q -l {build_name}.rpt {build_name}.ys]'",
            "yosys -q -l {build_name}.rpt {build_name}.ys",

            "echo 'Executing [nextpnr-ecp5 --json {build_name}.json --lpf {build_name}.lpf --textcfg {build_name}.config --basecfg {basecfg} --{architecture} --freq {freq_constraint}]'",
            ...
            "echo 'Executing [ecppack --input {build_name}.config --bit {build_name}.bit --svf {build_name}.svf --idcode {idcode}]'",
            "ecppack --input {build_name}.config --bit {build_name}.bit --svf {build_name}.svf --idcode {idcode}"
        ]

        self.freq_constraints = dict()

    def build(self, platform, fragment, build_dir="build", build_name="top",
              toolchain_path=None, run=True, idcode=None, **kwargs):
        ...
        script = _build_script(False, self.build_template, build_name,
                               architecture, basecfg, freq_constraint, idcode)
